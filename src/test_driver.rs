...
pub fn option_is_some(){
  ...
  //         })`
  {
    ...
    assert!(
        Some(31u8).is_some() == (match Some(31u8) { Some(_) => true, None => false, })
    );
    assert!(
        Some(119u8).is_some() == (match Some(119u8) { Some(_) => true, None => false, })
    );
    assert!(
        Some(27u8).is_some() == (match Some(27u8) { Some(_) => true, None => false, })
    );
    assert!(
        Some(38u8).is_some() == (match Some(38u8) { Some(_) => true, None => false, })
    );
  }
  eprint!("... ");
  eprintln!("✓\n");
...
pub fn option_is_none(){
  ...
  //         })`
  {
    ...
    assert!(
        Some(252u8).is_none() == (match Some(252u8) { Some(_) => false, None => true, })
    );
    assert!(
        Some(77u8).is_none() == (match Some(77u8) { Some(_) => false, None => true, })
    );
    assert!(
        Some(89u8).is_none() == (match Some(89u8) { Some(_) => false, None => true, })
    );
    assert!(
        Some(83u8).is_none() == (match Some(83u8) { Some(_) => false, None => true, })
    );
  }
  eprint!("... ");
  eprintln!("✓\n");
...
pub fn option_filter(){
  ...
  // __Postcondition:__ `Some(v).filter(f).is_some() == f(&v)`
  {
    ...
    assert!(
        Some(102u8).filter((| x : & u8 | * x > 128)).is_some() == (| x : & u8 | * x >
        128) (& 102u8)
    );
    assert!(
        Some(128u8).filter((| x : & u8 | * x > 128)).is_some() == (| x : & u8 | * x >
        128) (& 128u8)
    );
    assert!(
        Some(32u8).filter((| x : & u8 | * x > 128)).is_some() == (| x : & u8 | * x > 128)
        (& 32u8)
    );
  }
  eprint!("... ");
...
pub fn option_take(){
  ...
  //     }`
  {
    ...
    assert!({ let mut y = Some(5u8).clone(); y.take() == Some(5u8) && y.is_none() });
    assert!({ let mut y = Some(151u8).clone(); y.take() == Some(151u8) && y.is_none() });
    assert!({ let mut y = Some(148u8).clone(); y.take() == Some(148u8) && y.is_none() });
    assert!({ let mut y = Some(80u8).clone(); y.take() == Some(80u8) && y.is_none() });
  }
  eprint!("... ");
  eprintln!("✓\n");
}
...
pub fn option_as_mut(){
  ...
  //     }`
  {
    ...
    assert!(
        { let (v_unwrapped, mut v_mut) = (Some(1u8).unwrap().clone(), Some(1u8)); * v_mut
        .as_mut().unwrap() += 10; v_mut.unwrap() == v_unwrapped + 10 }
    );
    assert!(
...
pub fn core_ops_add_u32_add(){
  ...
  // __Postcondition:__ `(x + y) + z == x + (y + z)`
  {
    ...
    assert!(
        (1042550799u32 + 782482406u32) + 1561262534u32 == 1042550799u32 + (782482406u32 +
        1561262534u32)
    );
    assert!(
        (85693732u32 + 3249341552u32) + 233414786u32 == 85693732u32 + (3249341552u32 +
        233414786u32)
    );
    assert!(
        (1303862261u32 + 1982577543u32) + 923700650u32 == 1303862261u32 + (1982577543u32
        + 923700650u32)
    );
  }
  eprint!("... ");
...
pub fn core_ops_add_u32_checked_add(){
  ...
  // __Postcondition:__ `x.checked_add(y) == y.checked_add(x)`
  {
    ...
    assert!(
        1832355262u32.checked_add(2178268854u32) == 2178268854u32
        .checked_add(1832355262u32)
    );
    assert!(
        2102922986u32.checked_add(3056301062u32) == 3056301062u32
        .checked_add(2102922986u32)
    );
    assert!(
        1156900450u32.checked_add(3261964514u32) == 3261964514u32
        .checked_add(1156900450u32)
    );
  }
  eprint!("... ");
  // ## Left identity
...
pub fn core_ops_add_u64_add(){
  ...
  // __Postcondition:__ `x + y == y + x`
  {
    ...
    assert!(
        5687026623193332530u64 + 820890411863974613u64 == 820890411863974613u64 +
        5687026623193332530u64
    );
    assert!(
        10712410455245545096u64 + 7222348910621825969u64 == 7222348910621825969u64 +
        10712410455245545096u64
    );
    assert!(
        862407298443626099u64 + 12281928211865385330u64 == 12281928211865385330u64 +
        862407298443626099u64
    );
    assert!(
        5073377454086887523u64 + 9391899965784426052u64 == 9391899965784426052u64 +
        5073377454086887523u64
    );
  }
  eprint!("... ");
  // ## Left identity
...
pub fn core_ops_add_u64_checked_add(){
  ...
  // __Postcondition:__ `x.checked_add(y) == Some(u64::down(x.up() + y.up()))`
  {
    ...
    assert!(
        5430976214486531257u64.checked_add(4452159472817542807u64) ==
        Some(9883135687304074064u64)
    );
    assert!(
        3714166765535238078u64.checked_add(5568796798007876915u64) ==
        Some(9282963563543114993u64)
    );
    assert!(
        7018291680783791766u64.checked_add(3801163595538201440u64) ==
        Some(10819455276321993206u64)
    );
    assert!(
        759096659125148862u64.checked_add(13872589988617603284u64) ==
  ...
  // __Postcondition:__ `x.checked_add(y) == y.checked_add(x)`
  {
    ...
    assert!(
        6610704845120514490u64.checked_add(5641838204242065259u64) ==
        5641838204242065259u64.checked_add(6610704845120514490u64)
    );
    assert!(
        11525573352864736181u64.checked_add(7339786106222254945u64) ==
        7339786106222254945u64.checked_add(11525573352864736181u64)
    );
    assert!(
        9419931050420023483u64.checked_add(7711411145921816272u64) ==
...
pub fn core_ops_add_u128_add(){
  ...
  // __Postcondition:__ `panics!(x + y)`
  {
    ...
    assert!(
        panics!(184425210232649071852582189860066537521u128 +
        159469729460936646573622456627906172987u128)
    );
    assert!(
        panics!(330055022628157842902120943159899804011u128 +
        56732038160401495637301816482831681653u128)
    );
    assert!(
        panics!(336332491792630363572829843219173405916u128 +
        237816585063136717276706582935827735024u128)
    );
    assert!(
        panics!(109305549229489408126476677402562735799u128 +
  ...
  // __Postcondition:__ `x + 0 == x`
  {
    ...
    assert!(
        146899066460309722320481839646626579616u128 + 0 ==
        146899066460309722320481839646626579616u128
    );
    assert!(
        285707762915744874666619329362836493975u128 + 0 ==
        285707762915744874666619329362836493975u128
    );
    assert!(
        311661337007061384407241621334560279141u128 + 0 ==
        311661337007061384407241621334560279141u128
    );
    assert!(
        214165971470013110037714762279066109657u128 + 0 ==
        214165971470013110037714762279066109657u128
    );
    assert!(
        311131795382448448020321925594324656476u128 + 0 ==
  ...
  // __Postcondition:__ `0 + x == x`
  {
    ...
    assert!(
        0 + 186272985158529778037870003626382741622u128 ==
        186272985158529778037870003626382741622u128
    );
    assert!(
        0 + 304548722077211209366187599494906146860u128 ==
        304548722077211209366187599494906146860u128
    );
    assert!(
        0 + 13375174828960519415938220668212393962u128 ==
        13375174828960519415938220668212393962u128
    );
    assert!(
        0 + 214312118478724164359959175253743035993u128 ==
        214312118478724164359959175253743035993u128
    );
    assert!(
        0 + 43290313256206281229707599714978280658u128 ==
...
pub fn core_ops_add_u128_checked_add(){
  ...
  // __Postcondition:__ `x.checked_add(y) == y.checked_add(x)`
  {
    ...
    assert!(
        199433924173343872839465418920654472541u128
    ...
    assert!(
        289212290630215333669252338697091235710u128
    ...
    assert!(
        284518684882817033394032554823890700836u128
  ...
  // __Postcondition:__ `x.checked_add(0u128) == Some(x)`
  {
    ...
    assert!(
        335301091005109133454616108549665587307u128.checked_add(0u128) ==
        Some(335301091005109133454616108549665587307u128)
    );
    assert!(
        207586692813909776091453831710488124798u128.checked_add(0u128) ==
        Some(207586692813909776091453831710488124798u128)
    );
    assert!(
        140451055432199402458350061288231770804u128.checked_add(0u128) ==
        Some(140451055432199402458350061288231770804u128)
    );
    assert!(
        313066521221137356429217935298879591721u128.checked_add(0u128) ==
        Some(313066521221137356429217935298879591721u128)
    );
    assert!(
        77936528754177969417578059569092989353u128.checked_add(0u128) ==
  ...
  // __Postcondition:__ `0u128.checked_add(x) == Some(x)`
  {
    ...
    assert!(
        0u128.checked_add(113891871890069189995663307045975395387u128) ==
        Some(113891871890069189995663307045975395387u128)
    );
    assert!(
        0u128.checked_add(279706794151156716772881565568633583013u128) ==
        Some(279706794151156716772881565568633583013u128)
    );
    assert!(
        0u128.checked_add(18250993685945966765121698267293579222u128) ==
        Some(18250993685945966765121698267293579222u128)
    );
    assert!(
        0u128.checked_add(147603520736975998041042811953217031511u128) ==
        Some(147603520736975998041042811953217031511u128)
    );
    assert!(
        0u128.checked_add(247621885866340685968408890283294647766u128) ==
